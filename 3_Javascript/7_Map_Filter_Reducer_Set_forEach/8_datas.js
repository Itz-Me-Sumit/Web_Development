...
// Filtering :- it filters
const new_Prod = products.filter((product) => product.price > 50).sort((a,b) => b.price - a.price);
...
// Map & Filter.
const map_filter = products.filter( (product)=> product.price > 400 ).map((product) => ({name: product.name , price: product.price}));
console.log(map_filter);



// Map & reduce :- same output as if we only use reduce.
const map_reduce = products.map((product)=>({name:product.name , price: product.price})).reduce((accumulator , currentValue) => { return accumulator+currentValue.price} , 0);
console.log(map_reduce);


// filter & reduce
const filter_reduce = products.filter((product) => product.price>400).reduce((accumulator,currentValue)=>{ return accumulator+currentValue.price},0);
console.log(filter_reduce);
